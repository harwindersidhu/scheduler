...
describe("Application", () => {
  ...
  it("loads data, edits an interview and keeps the spots remaining for Monday the same", async () => {
    ...
    // 4. Check that the interview form is shown and change the name.
    expect(getByTestId("student-name-input")).toHaveValue("Archie Cohen");
    fireEvent.change(getByPlaceholderText(appointment, /enter student name/i), {
      target: { value: "Archie Archie" }
    });
    ...
    // 7. Wait until the Archie Cohen is displayed.
    await waitForElement(() => getByText(appointment, "Archie Archie"));
  ...
  it("shows the save error when failing to save an appointment", async () => {
    ...
    //4. Fill the form and click Save button.
    fireEvent.change(getByPlaceholderText(appointment, /enter student name/i), {
      target: { value: "Lydia Miller-Jones" }
    });
    ...
    //6. Wait until the "Could not save appointment." is displayed.
    await waitForElement(() => getByText(appointment, "Could not save appointment."));

    //7. Click close button.
    fireEvent.click(getByAltText(appointment, "Close"));

    //8. Check if we are back to form.
  ...
  it("shows the delete error when failing to delete an existing appointment", async () => {
    ...
    //8. Check if we are back to show mode of appointment.
